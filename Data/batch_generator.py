...
def batch_generator(images_names, batch_size=64, image_size=(576, 384)):
    '''
    images_names: List of names of images for a given video_type and series.
    ...
    random_indices = np.random.randint(
        low=0, high=len(images_names), size=batch_size)  # Generating random indices to pick images
    for ind in random_indices:

        # Reading and storing the images according to the random indices generated, after resizing them.

        image_name = images_names[ind]
